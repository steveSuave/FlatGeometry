...
class Line extends GeometryObject {
  ...
  void draw(Canvas canvas, Size size, double zoomScale) {
    ...
    if (isSelected) {
      ...
      canvas.drawCircle(Offset(start.x, start.y), controlPointRadius, controlPointPaint);
      canvas.drawCircle(Offset(end.x, end.y), controlPointRadius, controlPointPaint);
    }
  }
  ...
  bool _isPointNearLine(Offset point, double threshold) {
    ...
    final t = ((point.dx - start.x) * dx + (point.dy - start.y) * dy) / (lineLength * lineLength);
